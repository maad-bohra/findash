...
export default function TransactionsView({ transactions, onDelete, onAdd, deleting, categories = [] }) {
  ...
  // Build a lookup map for icon/color from dynamic categories
  const catMap = {}
  ...
  return (
    <div className="anim-fade-up" style={{ display: 'flex', flexDirection: 'column', gap: '1.4rem' }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        />
        {/* Dynamic category filter built from the categories prop */}
                ...
                {filtered.map(t => {
                  ...
                  return (
                    <tr key={t._id} className={styles.txRow}>
                      ...
                      <td style={{ color: 'var(--text-2)', fontSize: 13 }}>{fmtDate(t.date)}</td>
