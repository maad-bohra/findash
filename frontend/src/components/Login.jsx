...
export default function Login({ onLogin, onGoSignup }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (data.success) {
        onLogin({ email: data.user.email, username: data.user.username, avatar: data.user.avatar || null, currency: data.user.currency || 'INR', isAdmin: data.user.isAdmin || false }, data.token)
      } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <input
        ...
        onFocus={e => { e.target.style.borderColor='#4f8ef7'; e.target.style.boxShadow='0 0 0 3px rgba(79,142,247,.15)' }}
